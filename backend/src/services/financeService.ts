...
export interface DREPreviewResult extends DREResult {
  preview: boolean;
}

export function calcularPreco(purchasePrice: number, margin: number, taxRate: number): number {
...
export function calcularDRE(
  ...
  allocationMap: Record<string, number> | null = null
): DREResult {
  const inventoryMap: Record<string, number> = Object.fromEntries(
...
export function calcularDREPreview(
  roundConfig: RoundConfigInput,
  items: DREItem[],
  inventory: InventoryEntry[]
): DREPreviewResult {
...
export function gerarFeedback(dre: DREResult): string[] {
