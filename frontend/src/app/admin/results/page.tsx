...
const LINE_COLORS = ['#003087', '#f5a623', '#0066ff', '#16a34a', '#dc2626', '#7c3aed', '#0891b2', '#db2777'];
          ...
          return (
            <div key={entry.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{
                ...
                background: bgColors[i], border: `2px solid ${borderColors[i]}`,
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                fontSize: '24px', fontWeight: 800, color: borderColors[i],
              }}>
                {position}º
              </div>
            </div>
        ...
        {eventKeys.map((key) => {
          ...
          return (
            <div key={key} style={{ borderRadius: 10, border: '1px solid var(--cenc-gray-200)', padding: '12px 16px' }}>
              <p style={{ margin: '0 0 8px', fontSize: '13px', fontWeight: 700, color: 'var(--cenc-gray-800)' }}>{description}</p>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                {eventsForKey.map((e) => (
                  <span key={e.id} style={{
                    ...
                    {e.mitigated ? 'Mitigado' : 'Penalidade'} {e.store?.squad?.name ?? e.store?.name ?? '—'}
...
function AdminResultsContent() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/rounds/${selectedRoundId}/results`)
      ...
      .catch((err) => {
        toast.error(err instanceof Error ? err.message : 'Não foi possível carregar os resultados.');
        setResults([]);
      })
  ...
  }, [selectedRoundId, allRounds, toast]);
  ...
  if (loadingRounds) {
    return <div style={{ maxWidth: 1000, display: 'flex', flexDirection: 'column', gap: 20 }}><Skeleton variant="line" width="160px" height="28px" /><Skeleton variant="table" rows={4} /></div>;
  }
  if (loadError) return <ErrorMessage message={loadError} onRetry={loadRounds} />;
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: 160, gap: 8 }}>
        <div style={{ width: 56, height: 56, borderRadius: '50%', background: 'var(--cenc-blue-50)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="var(--cenc-blue-400)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
  ...
  return (
    <div style={{ maxWidth: 1000, display: 'flex', flexDirection: 'column', gap: 24 }} className="page-enter">
      ...
      {isNotClosed && (
        <div style={{ borderRadius: 14, border: '1px solid #fde047', background: '#fefce8', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 14 }}>
