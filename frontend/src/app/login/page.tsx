...
const CencosudLogo = () => (
  <svg width="56" height="56" viewBox="0 0 100 100" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <path d="M50 15C30.67 15 15 30.67 15 50C15 69.33 30.67 85 50 85C69.33 85 85 69.33 85 50C85 30.67 85 30.67 50 15Z" fill="#003087"/>
...
export default function LoginPage() {
  ...
  // Apply / persist dark mode to <html>
  useEffect(() => {
  ...
  async function onSubmit({ email, password }: LoginFormData) {
    ...
    try {
      ...
      // Fetch the session to discover the role and route accordingly
      const sessionRes = await fetch('/api/auth/session');
      const session = await sessionRes.json();
      const role = session?.user?.role as 'GAME_MASTER' | 'PLAYER' | 'OBSERVER' | undefined;
      const target =
        role === 'GAME_MASTER' ? '/admin' :
        role === 'PLAYER' ? '/dashboard' :
        role === 'OBSERVER' ? '/dashboard/ranking' :
        '/dashboard';
      router.push(target);
      router.refresh();
    } catch {
