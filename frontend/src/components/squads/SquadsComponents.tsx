...
import type { Squad, SquadUser, UserRecord, UserRole } from '@/components/types';
  ...
  async function onSubmit(data: SquadFormData) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.message ?? 'Erro ao criar squad');
      }
      const squad = await res.json();
      onSuccess(squad);
    } catch (err) {
      setServerError(err instanceof Error ? err.message : 'Erro ao criar squad');
    }
  }
      ...
      <div style={{ background: 'white', borderRadius: 20, boxShadow: '0 24px 64px rgba(0,0,0,0.18)', border: '1px solid var(--cenc-gray-200)', width: '100%', maxWidth: 440, margin: '0 16px', padding: 28, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <input style={fieldStyle(!!errors.name)} {...register('name')} autoFocus />
...
export function EditSquadModal({ squad, onSuccess, onCancel }: { squad: Squad; onSuccess: (s: Squad) => void; onCancel: () => void }) {
  ...
  async function onSubmit(data: SquadFormData) {
    setServerError('');
    try {
      const res = await fetch(`${API_BASE}/squads/${squad.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      });
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.message ?? 'Erro ao atualizar squad');
      }
      const updated = await res.json();
      onSuccess(updated);
    } catch (err) {
      setServerError(err instanceof Error ? err.message : 'Erro ao atualizar squad');
    }
  }
...
export function CreateUserModal({ squads, onSuccess, onCancel }: { squads: Squad[]; onSuccess: (u: UserRecord) => void; onCancel: () => void }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(4px)' }}>
      <div style={{ background: 'white', borderRadius: 20, boxShadow: '0 24px 64px rgba(0,0,0,0.18)', border: '1px solid var(--cenc-gray-200)', width: '100%', maxWidth: 480, margin: '0 16px', padding: 28, display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, gridColumn: '1/-1' }}>
              ...
              <input type="email" style={fieldStyle(!!errors.email)} {...register('email')} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, gridColumn: '1/-1' }}>
              ...
              <input type="password" style={fieldStyle(!!errors.password)} {...register('password')} />
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <select style={fieldStyle(!!errors.role)} {...register('role')}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              ...
              <select style={fieldStyle(false)} {...register('squadId')}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6, gridColumn: '1/-1' }}>
              ...
              <select style={fieldStyle(false)} {...register('cargo')}>
...
export function AddUserModal({
  ...
  async function handleAdd() {
    ...
    setServerError('');
    try {
      const res = await fetch(`${API_BASE}/squads/${squadId}/users`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      setServerError(err instanceof Error ? err.message : 'Erro ao adicionar usuário');
    } finally {
      setLoading(false);
    }
  }
      ...
      <div style={{ background: 'white', borderRadius: 20, boxShadow: '0 24px 64px rgba(0,0,0,0.18)', border: '1px solid var(--cenc-gray-200)', width: '100%', maxWidth: 440, margin: '0 16px', padding: 28, display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        {freeUsers.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <select value={selectedUserId} onChange={(e) => setSelectedUserId(e.target.value)} style={fieldStyle(false)}>
...
interface BulkError { index: number; email: string; reason: string }
interface BulkResult { created: UserRecord[]; errors: BulkError[]; password: string }
...
export function BulkCreateUsersModal({ squad, onSuccess, onCancel }: {
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch(`${API_BASE}/users/bulk`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ squadId: squad.id, count }),
      });
      if (!res.ok) {
        const err = await res.json();
        throw new Error(err.message ?? 'Erro ao gerar usuários');
      }
      const data = await res.json();
      setResult(data);
      setStep('result');
      if (data.created?.length > 0) {
        onSuccess(data.created as UserRecord[]);
      }
    } catch (err) {
      setServerError(err instanceof Error ? err.message : 'Erro ao gerar usuários');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.45)', backdropFilter: 'blur(4px)' }}>
      <div style={{ background: 'white', borderRadius: 20, boxShadow: '0 24px 64px rgba(0,0,0,0.18)', border: '1px solid var(--cenc-gray-200)', width: '100%', maxWidth: 500, margin: '0 16px', padding: 28, display: 'flex', flexDirection: 'column', gap: 20 }}>
        {step === 'config' ? (
          <>
            <div>
          ...
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, maxHeight: 260, overflowY: 'auto' }}>
              {result!.created.map((u) => (
                <div key={u.id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 10px', borderRadius: 8, background: '#f0fdf4', border: '1px solid #bbf7d0', fontSize: 12 }}>
                  <span style={{ color: '#16a34a', fontWeight: 700 }}>✓</span>
                  <span style={{ color: 'var(--cenc-gray-700)', fontFamily: 'monospace' }}>{u.email}</span>
                </div>
              ))}
              {result!.errors.map((e) => (
