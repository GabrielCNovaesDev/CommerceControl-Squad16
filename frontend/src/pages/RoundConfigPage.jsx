...
const SLA_TABLE      = [6, 5, 4, 3, 2, 1]; // index = serviceOperators (capped at 5)

const CAPEX_DEFS = [
  { key: 'capexSeguranca',    label: 'Segurança',        cost: 50000,
    desc: 'Monitoramento contra ataques. Risco sem CAPEX: 2+SLA dias sem venda.' },
  { key: 'capexBalanca',      label: 'Balança/Freezer',  cost: 75000,
    desc: 'Novos equipamentos (garantia). Elimina manutenção de R$400/mês.' },
  { key: 'capexRedes',        label: 'Redes',             cost: 80000,
    desc: 'Infraestrutura de rede. Risco sem CAPEX: 2+SLA dias sem venda.' },
  { key: 'capexSite',         label: 'Site',              cost: 65000,
    desc: 'Plataforma digital. Risco sem CAPEX: 1+SLA dias. +30% na licença do site.' },
  { key: 'capexSelfCheckout', label: 'Self Checkout',     cost: 80000,
    desc: '4 equipamentos. Adiciona licença R$320/mês. Risco sem CAPEX: 2+SLA dias.' },
  { key: 'capexMelhoria',     label: 'Melhoria Contínua', cost: 45000,
    desc: 'Automação de relatórios e processos. Sem penalidade direta de vendas.' },
];
...
function calcSlaFromOps(serviceOperators) {
...
function calcCapexTotal(vals) {
  const costs = { capexSeguranca: 50000, capexBalanca: 75000, capexRedes: 80000,
    ...
    <div className="rounded-xl bg-white border border-gray-200 shadow-sm">
      <div className="px-5 py-4 border-b border-gray-100">
        ...
        <p className="text-xs text-gray-400 mt-0.5">
          CSAT = min(1, Op.&nbsp;Caixa / 10) × Resultado&nbsp;Teste
        </p>
      </div>
      <div className="px-5 py-4 flex flex-col gap-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div className="flex items-center justify-between rounded-lg bg-purple-50 border border-purple-100 px-4 py-3">
            ...
            <div className="text-right text-xs text-purple-600 max-w-[120px] text-right leading-tight">
...
function CapexPanel({ register, control, errors }) {
  const watched = useWatch({
    control,
    name: ['capexSeguranca', 'capexBalanca', 'capexRedes', 'capexSite', 'capexSelfCheckout', 'capexMelhoria'],
  });
  const capexValues = Object.fromEntries(
    CAPEX_DEFS.map((d, i) => [d.key, watched[i]])
  );
    ...
    <div className="rounded-xl bg-white border border-gray-200 shadow-sm">
      <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-400 mt-0.5">
            Investimentos únicos deduzidos do capital inicial. Afetam SLA, licenças e manutenção.
          </p>
        </div>
        {totalCapex > 0 && (
      ...
      <div className="px-5 py-4 flex flex-col gap-4">
        <div className="flex items-end gap-3 max-w-xs">
          <div className="flex flex-col gap-1 flex-1">
            ...
            <input type="number" min="0"
              className={`rounded-lg border px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500
                ${errors.numPdvs ? 'border-red-400 bg-red-50' : 'border-gray-300'}`}
              {...register('numPdvs')} />
        ...
        <div className="rounded-lg bg-gray-50 border border-gray-200 overflow-hidden text-sm">
          ...
          {[
            ...
            capexSCO ? ['Self Checkout (4 × R$80)', scoCost] : null,
        ...
        <div className={`flex items-center justify-between rounded-lg px-4 py-2.5 text-sm border
          ${capexBalanca ? 'bg-green-50 border-green-200' : 'bg-yellow-50 border-yellow-200'}`}>
          <span className={capexBalanca ? 'text-green-700' : 'text-yellow-800'}>
            Manutenção de equipamentos
            {capexBalanca ? ' — cancelada (CAPEX Balança/Freezer)' : ''}
          </span>
          <span className={`font-semibold ${capexBalanca ? 'text-green-600 line-through' : 'text-yellow-800'}`}>
            {formatCurrency(400)}
          </span>
          {capexBalanca && (
            <span className="ml-2 text-green-700 font-semibold">{formatCurrency(0)}</span>
          )}
        </div>
      </div>
    </div>
...
function CashSummaryPanel({ initialCapital, currentCash, products, control }) {
  ...
  const numPdvs         = Number(useWatch({ control, name: 'numPdvs' })) || 6;
  const capexSite       = capexFields[3];
  const capexSeguranca  = capexFields[0];
  const capexSCO        = capexFields[4];
  const capexBalanca    = capexFields[1];

  const capexValues = Object.fromEntries(CAPEX_DEFS.map((d, i) => [d.key, capexFields[i]]));
  const capexCost   = calcCapexTotal(capexValues);

  const stockCost = watchedItems.reduce((sum, item, idx) => {
    ...
    <div className="grid grid-cols-[1fr_auto_auto_auto] gap-3 items-start py-3 border-b border-gray-100 last:border-0">
      ...
      <div className="w-32">
        ...
        <input type="number" step="0.1" min="0"
          className={`w-full rounded-lg border px-2.5 py-1.5 text-sm outline-none focus:ring-2 focus:ring-blue-500
...
export default function RoundConfigPage() {
  ...
  async function load() {
    ...
    try {
      ...
      reset({
        ...
        capexSelfCheckout: false,
        capexMelhoria:     false,
        items: prods.map((p) => ({ productId: p.id, margin: 0.12, salesVolume: 1 })),
      });
  ...
  function collectPayload(values) {
    return {
    ...
    <PlayerLayout>
      <div className="max-w-3xl flex flex-col gap-6">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
          ...
          {/* CAPEX */}
          <CapexPanel register={register} control={control} errors={errors} />
          ...
          <div className="rounded-xl bg-white border border-gray-200 shadow-sm">
            <div className="px-5 py-4 border-b border-gray-100">
              <h2 className="text-sm font-semibold text-gray-700">Outros Gastos</h2>
              <p className="text-xs text-gray-400 mt-0.5">
                Despesas adicionais (ex.: marketing, consultorias). Folha, licenças e CAPEX
            ...
            <>
              {preview.cashSummary && (
                <div className={`rounded-xl border px-5 py-4 flex flex-col gap-3
                  ...
                  <div className="flex items-center justify-between">
                    <h3 className="text-sm font-semibold text-gray-700">Resumo da Simulação</h3>
                    <span className={`text-xs font-semibold px-2 py-0.5 rounded-full
                  ...
                  <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm">
                    {[
                      ...
                      ['Licenças',      preview.cashSummary.licensing, false],
                      ['Manutenção',    preview.cashSummary.maintenance, false],
                      ['Saldo após investimentos', preview.cashSummary.balance,
                        !preview.cashSummary.cashOk],
                    ].map(([label, val, red]) => (
                      <div key={label} className="flex justify-between">
                        <span className="text-gray-500">{label}</span>
                        <span className={`font-medium ${red ? 'text-red-700' : ''}`}>
                          {formatCurrency(val)}
                        </span>
                      </div>
                    ))}
                    {preview.cashSummary.interestPenalty > 0 && (
                      <div className="col-span-2 flex justify-between border-t border-red-200 pt-2 mt-1 text-red-700">
                        <span className="text-xs">Juros sobre excesso (12%)</span>
                        <span className="font-semibold text-xs">
                          {formatCurrency(preview.cashSummary.interestPenalty)}
                        </span>
                      </div>
                    )}
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      <span className="text-gray-500">SLA de serviços</span>
                      <span className="font-medium text-purple-700">
                        {preview.cashSummary.sla} dia(s)
                      </span>
                    </div>
                  </div>
                </div>
              )}
              <DREPreview dre={preview.dre} feedbacks={preview.feedbacks} />
            </>
          )}
