    ...
    Promise.allSettled([
      ...
      if (resultResp.status === 'fulfilled') {
        ...
        if (rankingResp.status === 'fulfilled') {
          const storeName = (resultResp.value as FinancialResult).store?.name;
          const entry = (rankingResp.value as RankingEntry[]).find((r) => r.storeName === storeName);
          if (entry) setRankingPosition(entry.position);
        }
      } else {
        const status = (resultResp.reason as { response?: { status?: number } })?.response?.status;
        if (status === 404) {
          setNoResult(true);
        }
      }
