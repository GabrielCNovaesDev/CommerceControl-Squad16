...
function ProductModal({
  ...
  return (
    <div style={{ position:'fixed',inset:0,zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.45)',backdropFilter:'blur(4px)' }}>
      <div className="animate-scale-in" style={{ background:'white',borderRadius:20,boxShadow:'0 24px 64px rgba(0,0,0,0.18)',border:'1px solid var(--cenc-gray-200)',width:'100%',maxWidth:520,margin:'0 16px',padding:28,display:'flex',flexDirection:'column',gap:20 }}>
        <div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} style={{ display:'flex',flexDirection:'column',gap:14 }}>
          <div style={{ display:'flex',flexDirection:'column',gap:6 }}>
            ...
            <input className="input-cenc" style={fs(!!errors.name)} {...register('name')} autoFocus={!isEdit} />
          ...
          <div style={{ display:'flex',flexDirection:'column',gap:6 }}>
            ...
            <input type="number" step="0.01" min="0.01" className="input-cenc" style={fs(!!errors.purchasePrice)} {...register('purchasePrice')} />
          ...
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12 }}>
            {[{label:'Imposto (0–1)',key:'taxRate',err:errors.taxRate},{label:'Quebra (0–1)',key:'breakageRate',err:errors.breakageRate},{label:'Aging (0–1)',key:'agingRate',err:errors.agingRate}].map(({label,key,err}) => (
              <div key={key} style={{ display:'flex',flexDirection:'column',gap:6 }}>
                ...
                <input type="number" step="0.001" min="0" max="1" className="input-cenc" style={fs(!!err)} {...register(key as keyof ProductFormData)} />
          ...
          <div style={{ display:'flex',flexDirection:'column',gap:6 }}>
            ...
            <input type="number" min="0" step="1" className="input-cenc" style={fs(!!errors.mixAvailable)} {...register('mixAvailable')} />
...
function DeleteModal({
  ...
  return (
    <div style={{ position:'fixed',inset:0,zIndex:50,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.45)',backdropFilter:'blur(4px)' }}>
      <div className="animate-scale-in" style={{ background:'white',borderRadius:20,boxShadow:'0 24px 64px rgba(0,0,0,0.18)',border:'1px solid var(--cenc-gray-200)',width:'100%',maxWidth:440,margin:'0 16px',padding:28,display:'flex',flexDirection:'column',gap:20 }}>
...
export default function ProductsManagementPage() {
  ...
  return (
    <AdminLayout>
      ...
      {createOpen && <ProductModal onSuccess={handleCreated} onCancel={() => setCreateOpen(false)} />}
      {editProduct && <ProductModal product={editProduct} onSuccess={handleUpdated} onCancel={() => setEditProduct(null)} />}
      {deleteProduct && <DeleteModal product={deleteProduct} onConfirm={handleDeleteConfirm} onCancel={() => { setDeleteProduct(null); setDeleteError(''); }} loading={deletingProduct} error={deleteError} />}
    </AdminLayout>
  );
}
