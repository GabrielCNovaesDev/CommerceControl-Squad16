...
const ROLE_LABEL = { PLAYER: 'Jogador', GAME_MASTER: 'Game Master', OBSERVER: 'Observador' };
...
function CreateSquadModal({ onSuccess, onCancel }) {
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      setServerError(err.response?.data?.message ?? 'Erro ao criar squad');
    }
  }
...
function EditSquadModal({ squad, onSuccess, onCancel }) {
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm({
    resolver: zodResolver(createSquadSchema),
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      setServerError(err.response?.data?.message ?? 'Erro ao atualizar squad');
    }
  }
...
function CreateUserModal({ squads, onSuccess, onCancel }) {
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm({
...
function AddUserModal({ squadId, squadName, freeUsers, onSuccess, onCancel }) {
  ...
  async function handleAdd() {
    ...
    try {
      ...
      setServerError(err.response?.data?.message ?? 'Erro ao adicionar usuário');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-xl shadow-xl border border-gray-200 w-full max-w-sm mx-4 p-6 flex flex-col gap-5">
        ...
        <div className="flex gap-3 justify-end">
          ...
          {freeUsers.length > 0 && (
            <Button type="button" onClick={handleAdd} loading={loading} disabled={!selectedUserId}>
              Adicionar
            </Button>
          )}
        </div>
                ...
                <td className="py-1.5 text-right">
                  {user.leader ? (
                    <span
                      ...
                      title="Transfira a liderança antes de remover este usuário do squad"
                    >
                      Remover
                    </span>
                  ) : (
    ...
    <AdminLayout>
      ...
      {createSquadOpen && (
        <CreateSquadModal
          ...
          onCancel={() => setCreateSquadOpen(false)}
        />
      )}
