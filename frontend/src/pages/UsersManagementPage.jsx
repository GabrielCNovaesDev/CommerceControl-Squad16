...
const ROLE_BADGE = { PLAYER: 'blue', GAME_MASTER: 'green', OBSERVER: 'gray' };
...
function EditUserModal({ user, squads, onSuccess, onCancel }) {
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm({
...
export default function UsersManagementPage() {
  ...
  const filtered = users.filter((u) => {
    ...
    return (
      ...
      (u.squadId && squadMap[u.squadId]?.toLowerCase().includes(q))
    );
  });
      ...
      <div className="max-w-5xl flex flex-col gap-6">
        ...
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              Crie e gerencie contas de jogadores e administradores.
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-gray-200 bg-white shadow-sm px-6 py-12 flex flex-col items-center gap-2">
            ...
            {!filter && (
              <p className="text-sm text-gray-400">Crie o primeiro usuário para começar.</p>
            )}
          </div>
        ) : (
